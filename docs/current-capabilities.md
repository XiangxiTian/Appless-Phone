...
| 天气/本地 | `weather.query` | `今天深圳的天气怎样` / `帮我查明天深圳天气` | 固定入口复用高德天气适配；city 与 dayOffset 从会话解析，实时或未来三天天气直接显示在对话中，含来源时间 | `read` | 复用高德 key | 高德天气通常不需要 VPN | 否 | C13；weatherSummary 文本输出，不使用动态工具结果组件 |
