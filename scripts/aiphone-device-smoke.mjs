...
if (listCases) {
  ...
  if (runPublicPersona) {
    console.log(JSON.stringify(publicPersonaCases.map((testCase) => ({
      ...
      retired: true,
      migratedTo: '用户管理 > 关于你',
      requires: ['manual verification through 用户管理 > 关于你'],
...
if (runPublicPersona) {
  console.error('Legacy --public-persona smoke is retired; verify 用户管理 > 关于你 manually (manual-only). No device action was performed.');
  process.exit(2);
}
if (runGmailSendManual) {
